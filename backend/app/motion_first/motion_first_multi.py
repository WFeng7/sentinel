...
def _build_cmd(args, cam) -> list[str]:
    ...
    if args.sqs_queue_url:
        cmd.extend(["--sqs-queue-url", args.sqs_queue_url])
    if args.yolo:
...
def main():
    ...
    parser.add_argument(
        "--cameras",
        default=str(Path(__file__).resolve().parents[3] / "backend" / "cameras_cache.txt"),
    ...
    parser.add_argument("--sqs-queue-url", default=os.environ.get("SENTINEL_SQS_QUEUE_URL"))
