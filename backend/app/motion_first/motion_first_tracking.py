...
class IncidentWindowProcessor:
    def __init__(
        ...
        *,
        ...
        self.sqs_queue_url = sqs_queue_url or os.environ.get("SENTINEL_SQS_QUEUE_URL")
    ...
    def process(
        ...
        *,
        ...
        if self._rate_limited():
            ...
            print(f"[motion_first] {self.camera_id} | Incident suppressed (rate limit).")
            return
        ...
        print(f"[motion_first] {self.camera_id} | Posting incident {event_id}")
        ...
        if self.sqs_queue_url:
            try:
                ...
                client.send_message(
                    ...
                    MessageAttributes={
                        "camera_id": {"DataType": "String", "StringValue": str(self.camera_id)},
                        "event_id": {"DataType": "String", "StringValue": str(event_id)},
                    },
                )
            except Exception as exc:
                now = time.time()
                if now - self._last_post_error_ts > 30:
                    print(f"[motion_first] {self.camera_id} | Failed to send SQS message: {exc}")
                    self._last_post_error_ts = now
...
def main():
    ...
    p.add_argument("--sqs-queue-url", default=os.environ.get("SENTINEL_SQS_QUEUE_URL"),
                   help="SQS queue URL for incident notifications")
    args = p.parse_args()
